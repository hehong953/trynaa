...
async function doSearch(page = 1) {
    ...
    const res = await fetch(`/api/search?word=${encodeURIComponent(word)}&lang=ko&page=${page}`);
...
async function loadEntry(id) {
    const res = await fetch(`/api/entry/ko/${id}`);
...
async function loadNextQuiz() {
    ...
    const res = await fetch('/api/quiz/random?lang=ko');
